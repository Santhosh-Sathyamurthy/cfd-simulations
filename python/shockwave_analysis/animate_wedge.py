...
ver = 1
Ma_value = 2
plot_types = ["vorticity", "mach","density"]
dpi = 200
simulation_duration = 10  # seconds of final video duration

def make_video(plot_type):
    output_dir = f"v{str(ver)}_ma_{str(Ma_value)}/{plot_type}_frames/"
    output_file = f"outputs/mp4/v{str(ver)}_{plot_type}_ma_{str(Ma_value)}.mp4"
